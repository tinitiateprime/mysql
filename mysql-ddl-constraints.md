...
# DDL - Constraints
* In MySQL, Data Definition Language (DDL) constraints are used to enforce rules on the structure of a database table, ensuring data integrity and consistency by enforcing certain conditions on the data being inserted, updated, or deleted in the table.
* These constraints are applied to columns when the table is created or altered.

## Here are the common types of DDL constraints in MySQL:
### NOT NULL Constraint:
...
MODIFY COLUMN EP_ID INT NOT NULL;
...
* Note: MySQL version 8.0.16 and later supports CHECK constraints.
```sql
